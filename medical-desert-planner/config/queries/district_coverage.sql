...
district_pop AS (
  SELECT
    d.district_key,
    sp.population / COUNT(*) AS estimated_population
  FROM district_dim d
  JOIN state_pop sp
    ON regexp_replace(UPPER(d.statename), '[^A-Z]', '') = sp.state_key
  GROUP BY d.district_key, sp.population
),
combined AS (
  SELECT
    ...
    n.insurance_pct,
    CASE
      WHEN dp.estimated_population > 0
      THEN (
  ...
  LEFT JOIN district_pop dp ON d.district_key = dp.district_key
  LEFT JOIN nfhs n
...
SELECT
  ...
  ROUND(estimated_population, 0) AS estimated_population,
